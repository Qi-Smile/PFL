...
def train_moe(args, shared_model, private_model, data_loader, optimizer, loss_fun, device):
    torch.manual_seed(0)
    private_model.train()
    shared_model.eval()
    loss_all = 0
    ...
    for data, target in data_loader:
        data = data.to(device).float()
        target = target.to(device).long()
        private_output = private_model(data)
        shared_output = shared_model(data)
        output = args.private_ratio*private_output+(1-args.private_ratio)*shared_output
        loss = loss_fun(output, target)
        loss_all += loss.item()
        total += target.size(0)
        pred = output.data.max(1)[1]
...
def test_moe(args, shared_model, private_model, data_loader, loss_fun, device):
    shared_model.eval()
    private_model.eval()
    loss_all = 0
    total = 0
